...
async function startLocalTask() {
  ...
  childProcess.execSync(`rsync -r --ignore-existing ${projectRoot}/packages/chat/dist/config/* ${zoweChatHome}/config`, {
    stdio: 'inherit',
  });
  childProcess.execSync(`rsync -r --ignore-existing ${projectRoot}/packages/chat/dist/config/plugin.yaml ${zoweChatPluginHome}`, {
    stdio: 'inherit',
  });
  console.log('Done!');
...
async function packagingTask() {
  try {
    ...
    if (process.env.RELEASE_TYPE === undefined || ['beta', 'ga'].includes(process.env.RELEASE_TYPE.toLowerCase()) === false) {
      // beta, ga
