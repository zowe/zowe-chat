...
import { logger, IMessage, IMessageType, IChatToolType, IExecutor, config, ChatHandler, IBotOption, IBotLimit, ICommand, IMsteamsBotLimit,
    IMattermostBotLimit, ISlackBotLimit, ChatPrincipal, Util } from '@zowe/chat';
...
class ZosFileHandler extends ChatHandler {
    ...
    async getFile(command: ICommand, executor: IExecutor): Promise<IMessage[]> {
        ...
        try {
            ...
            const response: IZosFilesResponse = await List.fileList(session, path,
            ...
            if (response.apiResponse.returnedRows === 1) {// Only 1 record (path is a specific file, show overview view.
                messages = this.view.getDetail(response.apiResponse.items, executor);
            } else { // Path is is directory.
                const files = [];
                if (fileName) { // Positional argument is provided.
                    if (fileName.indexOf('*') === -1 && fileName.indexOf('?') === -1) { // No wildcard in filename, exactly match
                ...
                } else { // No Positional argument, show overview view.
    ...
    async getMounts(command: ICommand, executor: IExecutor): Promise<IMessage[]> {
        ...
        try {
            ...
            let fileSystems = [];
            let response;
            if (mountPoint !== null) { // mount point is provided, query with mount point
                ...
                } else {// filter all file systems with fileSystemName.
                    fileSystems = this.filterFileSystemByName(response.apiResponse.items, fileSystemName, Number(limit).valueOf());
                }
            } else { // mount point is not provided.
                if (fileSystemName !== null) { // positional argument fileSystemName is provided.
                    if (fileSystemName.indexOf('*') === -1
                        && fileSystemName.indexOf('?') === -1) {
                        ...
                        fileSystems = response.apiResponse.items;
                    } else { // positional argument fileSystemName with wildcard is provided, query all file systems.
                        response = await List.fs(session, {
                            path: null,
                            fsname: null,
                            maxLength: 0,
                ...
                } else { // fileSystemName and mount point are all not provided.
                    response = await List.fs(session, {
                        path: null,
                        fsname: null,
                        maxLength: Number(limit).valueOf(),
