...
class ZosDatasetMsteamsView extends ChatMsteamsView {
    ...
    try {
      ...
      for (const dataset of datasets) {
        ...
        cardObject.body.push(
          super.createColumnSet(
            `${isPartitioned === true ? String.fromCodePoint(0x1f5c2) : String.fromCodePoint(0x1f4c3)} **${i18next.t(
              'command.dataset.list.status.name',
              { ns: 'ZosMessage' },
            )}:** ${dataset.dsname}`,
            `**${i18next.t('command.dataset.list.status.organization', { ns: 'ZosMessage' })}:** ${
    ...
    try {
      ...
      cardObject.body.push(
        super.createColumnSet(
          ...
          `**${i18next.t('command.dataset.list.detail.lastReferenceDate', { ns: 'ZosMessage' })}:** ${
  ...
  getMemberOverview(members: Record<string, unknown>[], executor: IExecutor, command: ICommand): IMessage[] {
    ...
    try {
      ...
      for (const member of members) {
        cardObject.body.push(
          super.createColumnSet(
            `${String.fromCodePoint(0x1f4c3)} **${i18next.t('command.dataset.list.member.name', { ns: 'ZosMessage' })}:** ${
              member.member
            }`,
            '',
            true,
  ...
  getMemberDetail(members: Record<string, unknown>[], executor: IExecutor): IMessage[] {
    ...
    try {
      ...
      cardObject.body.push(
        super.createColumnSet(
          `**${i18next.t('command.dataset.list.memberDetail.creationTime', { ns: 'ZosMessage' })}:** ${
            member.ec4datextx ? member.ec4datextx : ''
          }`,
      ...
      cardObject.body.push(
        super.createColumnSet(
          `**${i18next.t('command.dataset.list.memberDetail.currentRecords', { ns: 'ZosMessage' })}:** ${
            member.cnorc ? member.cnorc : ''
          }`,
          `**${i18next.t('command.dataset.list.memberDetail.initialRecords', { ns: 'ZosMessage' })}:** ${
