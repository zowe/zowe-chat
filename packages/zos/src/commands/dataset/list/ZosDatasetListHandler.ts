...
import { logger, Util, IMessage, IMessageType, IChatToolType, IExecutor, config, ChatHandler, IBotOption, IBotLimit, ICommand, IMsteamsBotLimit,
    IMattermostBotLimit, ISlackBotLimit, ChatPrincipal } from '@zowe/chat';
        ...
        try {
            ...
            if (name === null && dsNameLevel === null ) {
                return messages = [{
                    ...
                    message: i18next.t('common.error.missing.argumentOrOption', { argumentName: 'dataset name', optionName: 'dsname-level', ns: 'ZosMessage' }),
                }];
            }
            ...
            if (name !== null) {
                resOfQueryWithName = await List.dataSet(session, encodeURIComponent(name),
                        { volume: volumeSerial, attributes: true, start: start, maxLength: Number(limit).valueOf() });
            }
            ...
            if (resOfQueryWithName !== null && resOfQueryWithDsNameLevel !== null) {
                if (resOfQueryWithName.apiResponse.items.length > 0
                    && resOfQueryWithDsNameLevel.apiResponse.items.length > 0) {
                ...
                } else if (resOfQueryWithName.apiResponse.items.length > 0) { // Only query with dsname level get result.
    ...
    async getDatasetMember(command: ICommand, executor: IExecutor): Promise<IMessage[]> {
        ...
        try {
            ...
            const response: IZosFilesResponse = await List.allMembers(session, datasetName,
                    { pattern: name, attributes: true, maxLength: Number(limit).valueOf() });
            ...
            if (name !== null
                && name.indexOf('*') === -1
                && name.indexOf('%') === -1
                && response.apiResponse.returnedRows === 1) {
