...
class ZosCommandIssueHandler extends ChatHandler {
  private view: ZosCommandIssueMattermostView | ZosCommandIssueMsteamsView | ZosCommandIssueSlackView = null;
  ...
  public async issueConsoleCommand(command: ICommand, executor: IExecutor): Promise<IMessage[]> {
    ...
    try {
      ...
      if (commandString === null || commandString === undefined || ('' + commandString).trim() === '') {
        return (messages = [
          {
