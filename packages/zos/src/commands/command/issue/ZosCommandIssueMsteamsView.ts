...
import { logger, Util, ChatMsteamsView, IBotOption, IExecutor, IMessage, IMessageType, IMsteamsBotLimit } from '@zowe/chat';
...
class ZosCommandIssueMsteamsView extends ChatMsteamsView {
  ...
  getOutput(issueResponse: IConsoleResponse, executor: IExecutor): IMessage[] {
    ...
    try {
      ...
      // Add command output
      if (
        issueResponse.commandResponse === null ||
        ...
        headerMessage =
          i18next.t('command.cmd.issue.console.cmdResponseOutput', { executorName: executor.name, ns: 'ZosMessage' }) +
          `\n\n` +
        ...
        cardObject.body.push({
          type: 'TextBlock',
          text:
            `${i18next.t('command.cmd.issue.console.cmdResponseUrl', { executorName: executor.name, ns: 'ZosMessage' })}: ` +
